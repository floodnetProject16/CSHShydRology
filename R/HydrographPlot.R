#' Plot Hydrograph
#'
#' HydrographPlot creates a hydrograph plot for simulated, observed, and inflow 
...
#' HydrographPlot(flows=df,winter.shading=F) 
#' 
#' with different labels
#' HydrographPlot(flows=df,winter.shading=F,flow.labels=c("simulated","observed")) 
#' 
#' # with a few more options turned on
#' HydrographPlot(flows=df,precip=precip)
#' 
#' # increase the plot ranges to separate flows and precip; add a legend box
#' HydrographPlot(flows=df,precip=precip,range.mult.flow=1.7,range.mult.precip=2,leg.box=T)
#' }
#'  
#' @export HydrographPlot
HydrographPlot <- function(flows=NULL,precip=NULL,prd=NULL,winter.shading=F,range.mult.flow=NULL,range.mult.precip=1.5,
  ...
  if (!(is.null(prd))) {
    ...
    if (nrow(DateSubset(flows,prd)) == 0) {
  ...
  # subset data
  flows <- CSHShydRology::DateSubset(flows,prd)
  
  if (!(is.null(precip))) {
    precip <- DateSubset(precip,prd)
    if (nrow(precip) == 0) {
  ...
  graphics::plot(flows$Date,flows[,2],xlab="Date",ylab=ylabel,
  ...
  if (!(is.null(precip))) {
    graphics::par(new=T)
    precip.col <- "#0000FF64"
    graphics::plot(precip$Date,precip[,2],col=precip.col,lty=1,lwd=1,
         type='h',ylim=rev(c(0,max(precip[,2],na.rm=T)*range.mult.precip)),xaxt='n',yaxt='n',
         xlab="",ylab="")
    graphics::axis(4)
    graphics::mtext(sprintf("%s",precip.label),side=4,line=2.5)
  ...
  # add legend to plot
  graphics::legend(x=leg.pos,legend=leg.items,lty=leg.lty,col=leg.cols,
